...
final class SpoonacularAPI {
    ...
    func searchRecipes(query: String, number: Int = 20) async throws -> [RecipeSearchItem] {
        var comps = URLComponents(
            url: SpoonacularConfig.baseURL.appendingPathComponent("/recipes/complexSearch"),
    ...
    func searchRecipes(cuisine: String, number: Int = 20) async throws -> [RecipeSearchItem] {
        var comps = URLComponents(
            url: SpoonacularConfig.baseURL.appendingPathComponent("/recipes/complexSearch"),
            resolvingAgainstBaseURL: false
        )
        ...
        comps?.queryItems = [
            .init(name: "apiKey", value: SpoonacularConfig.apiKey),
            .init(name: "cuisine", value: cuisine),
            .init(name: "number", value: String(number))
        ]
    ...
    func recipeDetails(id: Int) async throws -> RecipeDetails {
        var comps = URLComponents(
            url: SpoonacularConfig.baseURL.appendingPathComponent("/recipes/\(id)/information"),
            resolvingAgainstBaseURL: false
        )
